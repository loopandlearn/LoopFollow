...
enum EndoReportGenerator {
                                   ...
                                   cfg: EndoReportConfig, ctx: CGContext)
    {
        ...
        // Draw a stable legend to avoid overlapping text inside a constrained vertical bar.
        let legendX = bx + bw + 8
        let legendY = by
        let legendSpacing: CGFloat = 12
        for (index, (pct, _, label)) in segs.filter({ $0.0 > 0.0 }).enumerated() {
            ...
            let textY = legendY + CGFloat(index) * legendSpacing
            (textStr as NSString).draw(at: CGPoint(x: legendX, y: textY), withAttributes: pa)
        }
