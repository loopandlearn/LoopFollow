...
enum EndoReportGenerator {
    ...
    private static func groupByDay(
        ...
        for r in bgData { let k=df.string(from:Date(timeIntervalSince1970:r.date)); bg[k,default:[]].append(r) }
        for r in boluses { let k=df.string(from:Date(timeIntervalSince1970:r.date)); bo[k,default:[]].append(r) }
        for r in smbs    { let k=df.string(from:Date(timeIntervalSince1970:r.date)); sm[k,default:[]].append(r) }
        for r in basals  { let k=df.string(from:Date(timeIntervalSince1970:r.date)); ba[k,default:[]].append(r) }
        for r in carbs   { let k=df.string(from:Date(timeIntervalSince1970:r.date)); ca[k,default:[]].append(r) }
        var result:[String:DayData]=[:  ]
        for k in bg.keys { result[k]=DayData(bg:bg[k]!,bolus:bo[k] ?? [],smb:sm[k] ?? [],basal:ba[k] ?? [],carbs:ca[k] ?? []) }
        return result
    }
    ...
    private static func drawSummaryPage(
        ...
        bgData:[ShareGlucoseData], agpData:[AGPDataPoint], tirData:[TIRDataPoint],
        ...
        if cfg.includeGlucoseSummary {
            y = sectionHdr("GLUCOSE SUMMARY", y:y+8, m:m, w:r.width, cfg:cfg, ctx:ctx)

            let gridW:CGFloat = r.width - m*2 - 158
            let cw = gridW/2 - 3; let ch:CGFloat=42
        ...
        if cfg.includeTherapySettings && hasSettings {
            ...
            if !cfg.basalRate.isEmpty    { rows.append(("Basal Rate",           cfg.basalRate)) }
            if !cfg.targetGlucose.isEmpty{ rows.append(("Target Glucose",       cfg.targetGlucose)) }
            y = settingsTable(rows, y:y+4, m:m, w:r.width, cfg:cfg, ctx:ctx)
        }
    ...
    private static func drawHero(ctx:CGContext, r:CGRect, cfg:EndoReportConfig, stats:ReportStats)->CGFloat {
        let h:CGFloat=95; let ac=accent(cfg); let ad=accentDark(cfg)
        ...
        for(i,l) in lines.enumerated() {
            ...
            (l as NSString).draw(at:CGPoint(x:r.width-26-sz.width,y:26+CGFloat(i)*13),withAttributes:a5)
        }
        return h
    }
    ...
    private static func statCard(_ label:String, val:String, x:CGFloat, y:CGFloat,
        ...
        let va:[NSAttributedString.Key:Any]=[.font:UIFont.boldSystemFont(ofSize:17),.foregroundColor:ac ? accent(cfg):C_INK]
    ...
    private static func drawTIRBar(tirData:[TIRDataPoint], stats:ReportStats,
        ...
        for(pct,clr,label) in segs {
            ...
            // Lowered minimum threshold to > 0.0 to catch "Low" and "Very Low"
            if pct > 0.0 {
                let ps = String(format:"%.0f%%",pct)
                let isTarget = (label == "In Range")
                let pa:[NSAttributedString.Key:Any] = [.font:UIFont.boldSystemFont(ofSize:7.5), .foregroundColor: isTarget ? accent(cfg) : C_SLATE]
                
                let textStr = "\(label) \(ps)"
                
                var textY = sy + sh/2 - 4
                // Push text down to prevent overlapping labels
                if textY < lastTextY + 10 { textY = lastTextY + 10 }
                
                (textStr as NSString).draw(at:CGPoint(x:bx+bw+6,y:textY),withAttributes:pa)
                lastTextY = textY
            }
            sy+=sh
        }
        
        // Expanded text and increased spacing for bottom targets
        let na:[NSAttributedString.Key:Any]=[.font:UIFont.systemFont(ofSize:6.5),.foregroundColor:C_SLATE]
        "Target: 70-180".draw(at:CGPoint(x:x+5,y:y+h-24),withAttributes:na)
        "Time in Tight Range: 70-140".draw(at:CGPoint(x:x+5,y:y+h-12),withAttributes:na)
    }
    ...
    private static func insulinSection(boluses:[MainViewController.bolusGraphStruct],
                                       smbs:[MainViewController.bolusGraphStruct],
                                       simpleVM:SimpleStatsViewModel, stats:ReportStats,
        ...
        let cw=(w-m*2)/3-4; let ch:CGFloat=38
        for(i,c) in cards.enumerated() {
    ...
    private static func nutritionSection(carbs:[MainViewController.carbGraphStruct],
                                         stats:ReportStats, cfg:EndoReportConfig,
                                         y:CGFloat, m:CGFloat, w:CGFloat, ctx:CGContext)->CGFloat {
    ...
    private static func metricTable(_ rows:[(String,String)], y:CGFloat, m:CGFloat,
        ...
        "METRIC".draw(at:CGPoint(x:m+6,y:cy+2),withAttributes:ha)
        "VALUE".draw( at:CGPoint(x:m+tw*0.62+6,y:cy+2),withAttributes:ha)
        cy+=hh
    ...
    private static func settingsTable(_ rows:[(String,String)], y:CGFloat, m:CGFloat,
        ...
        let ha: [NSAttributedString.Key: Any] = [.font: UIFont.boldSystemFont(ofSize: 7), .foregroundColor: accent(cfg), .kern: 0.4]
        "SETTING".draw(at: CGPoint(x: m + 6, y: cy + 3), withAttributes: ha)
        "SCHEDULE / VALUES".draw(at: CGPoint(x: m + tw * 0.35 + 6, y: cy + 3), withAttributes: ha)
        cy += headerH
        
        for (i, row) in rows.enumerated() {
            let lines = row.1.components(separatedBy: "\n").filter { !$0.trimmingCharacters(in: .whitespaces).isEmpty }
            let rh = Swift.max(18.0, CGFloat(lines.count) * 11.0 + 8.0)
            
            ctx.setFillColor((i % 2 == 0 ? C_WHITE : C_CLOUD).cgColor)
            ctx.fill(CGRect(x: m, y: cy, width: tw, height: rh))
            
            let ka: [NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: 8), .foregroundColor: C_INK]
            let va: [NSAttributedString.Key: Any] = [.font: UIFont.boldSystemFont(ofSize: 8), .foregroundColor: accent(cfg)]
            
            (row.0 as NSString).draw(at: CGPoint(x: m + 6, y: cy + 5), withAttributes: ka)
            
            var ly = cy + 5
            for line in lines {
                (line as NSString).draw(at: CGPoint(x: m + tw * 0.35 + 6, y: ly), withAttributes: va)
                ly += 11
            }
    ...
    private static func drawAGP(agpData:[AGPDataPoint], x:CGFloat, y:CGFloat,
        ...
        for(i,pt) in agpData.enumerated() { let p=CGPoint(x:tx(pt.timeOfDay),y:gy(pt.p95)); i==0 ? band.move(to:p):band.addLine(to:p) }
        for pt in agpData.reversed() { band.addLine(to:CGPoint(x:tx(pt.timeOfDay),y:gy(pt.p5))) }
        band.closeSubpath()
        ctx.setFillColor(accent(cfg).withAlphaComponent(0.10).cgColor); ctx.addPath(band); ctx.fillPath()

        var iqr=CGMutablePath()
        for(i,pt) in agpData.enumerated() { let p=CGPoint(x:tx(pt.timeOfDay),y:gy(pt.p75)); i==0 ? iqr.move(to:p):iqr.addLine(to:p) }
        for pt in agpData.reversed() { iqr.addLine(to:CGPoint(x:tx(pt.timeOfDay),y:gy(pt.p25))) }
        iqr.closeSubpath()
        ...
        var first=true
        for pt in agpData { let p=CGPoint(x:tx(pt.timeOfDay),y:gy(pt.p50)); first ? ctx.move(to:p):ctx.addLine(to:p); first=false }
        ctx.strokePath()
    ...
    private static func drawDayRow(ctx:CGContext, x:CGFloat, y:CGFloat, w:CGFloat, h:CGFloat,
        ...
        if !vals.isEmpty {
            ...
            let cols:[(String,CGFloat)]=[ ("Avg",0),("TIR",30),("Bolus",66),("Basal",106),("B:Bas",144) ]
            for(lbl,ox) in cols { (lbl as NSString).draw(at:CGPoint(x:statsX+ox,y:y+5),withAttributes:sa) }
            ...
            String(format:"%.1fU",totalBolus).draw(at:CGPoint(x:statsX+64, y:y+14),withAttributes:sv)
            (basalTotal>0 ? String(format:"%.1fU",basalTotal):"—").draw(at:CGPoint(x:statsX+104, y:y+14),withAttributes:sv)
            let ratioStr = total>0 ? String(format:"%.0f:%.0f",bolusPct,basalPct):"—"
            ratioStr.draw(at:CGPoint(x:statsX+142,y:y+14),withAttributes:acA)
        }

        let chartX=x+10; let chartW=w-190 
        ...
        if !dayData.basal.isEmpty {
            ...
            for (index, pt) in sorted.enumerated() { 
                ...
                if pt.basalRate > 0.05 {
                    let nextX = index < sorted.count - 1 ? tx(sorted[index+1].date) : chartX + chartW
                    if nextX - px > 16 { 
                         let rateStr = String(format: "%.1f", pt.basalRate)
                         let rA:[NSAttributedString.Key:Any]=[.font:UIFont.systemFont(ofSize:5),.foregroundColor:C_BASAL]
    ...
    private static func drawFooter(ctx:CGContext, r:CGRect, cfg:EndoReportConfig,
