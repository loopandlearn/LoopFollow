...
class GeneralSettingsViewController: FormViewController {
            ...
            <<< PushRow<String>("speakLanguage") { row in
                ...
                row.presentationMode = PresentationMode.presentModally(
                    controllerProvider: ControllerProvider.callback {
                        SelectorViewController<SelectorRow<PushSelectorCell<String>>> { _ in }
                    },
                    onDismiss: { vc in
    ...
    func updateSpeakBGSettingsVisibility() {
        ...
        for tag in ["speakLowBG", "speakProactiveLowBG", "speakHighBG"] {
    ...
    @objc func handleAppWillEnterForeground() {
        if let row = form.rowBy(tag: "speakBG") as? SwitchRow {
            row.value = UserDefaultsRepository.speakBG.value
            row.updateCell()
        }
    }
